...
function riftArgs(){return{runId:['rift',S?.rift?.layer||S?.riftLayer||0,S?.player?.cls||'',Math.floor(S?.time||0),Math.floor(S?.kills||0)].join(':')}}
...
window.openBoard=async function(){if(window.boardBusy)return;window.boardBusy=true;$id('boardSub').textContent='加载中...';$id('boardList').innerHTML='<div class="choice">正在读取大秘境最高层排行</div>';$id('leaderboard').classList.remove('hidden');if(S)S.paused=true;try{let r=await invokeFn('leaderboard',{method:'list'},{max:2});renderBoard(r.board);$id('boardSub').textContent='大秘境最高层前二十名，点击玩家查看职业、流派、时间、层数、技能和装备'}catch(e){if(window.isCaptchaError?.(e))showCaptchaBoard();else{$id('boardSub').textContent=e.code==='function_not_published'?'排行榜函数还未发布，请保存游戏后上线':'排行榜加载失败';$id('boardList').innerHTML=`<div class="choice">${esc(e.message||'请稍后重试')}</div>`}}finally{window.boardBusy=false}};
window.submitScore=async function(){if(!S?.player||S.mapId!=='rift'||!S.rift?.guardianKilled||S.rift._boardSubmitted||S.rift._boardSubmitting)return;S.rift._boardSubmitting=true;try{let r=await invokeFn('leaderboard',{method:'submit',args:riftArgs()},{max:1});renderBoard(r.board);S.rift._boardSubmitted=!!r.rank}catch(e){S.rift._boardSubmitted=false;if(window.isCaptchaError?.(e)){console.warn('秘境排行榜需要验证后提交:',e.message);showNotice?.('排行榜需要完成真人验证后再提交');return}console.error('秘境排行榜提交失败:',e.code,e.message,e.stack)}finally{S.rift._boardSubmitting=false}};
