...
window.submitScore=async function(){if(!S?.player||S.mapId!=='rift'||!S.rift?.guardianKilled||S.rift._boardSubmitted)return;try{let names=await playerNameFields();let r=await invokeFn('leaderboard',{method:'submit',args:{...riftArgs(),...names}});renderBoard(r.board);if(r.rank)S.rift._boardSubmitted=true;else setTimeout(()=>window.submitScore?.(),1200)}catch(e){S.rift._boardSubmitted=false;if(window.isCaptchaError?.(e)){console.warn('秘境排行榜需要验证后提交:',e.message);showNotice?.('排行榜需要完成真人验证后再提交');return}console.error('秘境排行榜提交失败:',e.code,e.message,e.stack)}};
