...
window.GameModules.season = (() => {
  ...
  async function addRunXp(run){await init();if(!started())return null;let old=level();await applyServerState(await callServer('runXp',run));let cur=season();return {gain:Math.max(0,cur.totalXp||0),level:cur.level,xp:cur.xp,next:need(cur.level),ups:Math.max(0,cur.level-old)}}
  async function grantLevel(target){await init();if(!started())await start();let old=level(),to=Math.min(cap(),Math.max(old,Math.floor(Number(target)||old)));let cur=season();cur.level=to;cur.xp=0;cur.totalXp=Math.max(cur.totalXp||0,0);state.seasons[CURRENT]=cur;await save();return{level:cur.level,ups:Math.max(0,to-old)}}
