...
window.GameModules.equipData = (() => {
  ...
  const UNIQUES = [
    ['unique-saint-nail','暗金·圣钉战槌','weapon','paladin',{critDmg:.45,armor:.18,attrDmg:{holy:.35},eliteDmg:.28,skillFreq:.15},{},'天谴重击：大蒜光环转为天谴重击，每秒引爆[总护甲×爆伤%]神圣伤害，秘境进度每10%连乘[x]9%'],
    ...
    ['unique-plague-bell','暗金·瘟疫铃','weapon','scytheMaiden',{dotDmg:.45,cooldown:.12,attrDmg:{shadow:.32},eliteDmgReduce:.06,crit:.10},{},'疫病加速：幽魂刃舞对DoT目标暴击伤害[x]55%，击杀精英加速DoT跳字100%'],
    ['unique-blaze-core','暗金·爆燃核心','weapon','mage',{critDmg:.40,attrDmg:{fire:.35},atkSpeed:.16,healthyDmg:.30,cooldown:.08},{},'无限火海：陨星碎片火海可无上限重叠，Boss在火海每秒火焰伤害独立连乘[x]12%(最高[x]120%)'],
    ...
    ['unique-dragon-heart','暗金·龙心余烬','chest',null,{hp:.28,armor:.16,eliteDmgReduce:.06,allRes:.10,thorns:2000},{},'龙心殉爆：击杀精英引发全屏殉爆，杂兵死亡额外提供1%秘境进度'],
    ['unique-elite-boots','暗金·精英猎手长靴','boots',null,{move:.20,armor:.12,crit:.08,slowResist:.40,bossDmgReduce:.05},{},'猎手增幅：全伤害随秘境进度提高，每10%进度获得[x]8%独立乘区，最高[x]80%'],
    ...
    ['unique-blood-plate','暗金·血契重甲','chest',null,{hp:.32,armor:.16,atkSpeed:.10,crit:.06},{physical:.24},'血契反击：生命每降低10%进化技能全伤害[x]10%(最高6层)，低于35%时全屏弹幕附带45%真吸血'],
    ['unique-clock-gloves','暗金·逆时针环','ring',null,{cooldown:.16,atkSpeed:.14,crit:.08,critDmg:.18},{arcane:.20},'逆时冷却：弹幕暴击时10%概率使冷却中技能CD减1秒'],
    ['unique-rose-mirror','暗金·蔷薇镜','amulet',null,{hp:.22,crit:.08,healBonus:.20,cooldown:.08},{lust:.26},'欲念蓄池：吸收60%承伤存入欲念池(上限3倍生命)，下次攻击以[x]220%喷溅；蔷薇镜像6件联动吸收72%、上限4.5倍生命、喷溅[x]300%'],
    ['unique-abyss-mask','暗金·深渊假面','helm',null,{critDmg:.30,crit:.10,skillDmgAdd:{scytheArc:.20,wraithBlade:.20},move:.08},{shadow:.26},'深渊斩杀：普通怪<25%/精英<15%/Boss<10%血量必定致命暴击斩杀'],
    ['unique-golem-soul','暗金·巨像魂核','ring',null,{armor:.15,hp:.24,thorns:2500,allRes:.08},{physical:.12},'岩盾守护：站立释放进化技能时获得全减伤，最高35%，总荆棘以150%加算到弹幕'],
    ['unique-demon-horn','暗金·魔王断角','helm',null,{critDmg:.28,bossDmg:.25,eliteDmgReduce:.05,atkSpeed:.08},{fire:.12},'魔王降临：Boss激活瞬间弹幕翻倍，对Boss独立连乘[x]60%'],
    ...
    ['unique-hunt-quiver','暗金·无尽箭匣','amulet',null,{atkSpeed:.20,crit:.10,skillDmgAdd:{daggerRain:.22,windCutter:.22},critDmg:.20},{physical:.20},'弹幕分裂：匕首雨/风裂刃按攻速乘算分裂，每10%额外攻速弹幕+20%(最多+80%)'],
  ];
  const SET_FAMILIES = [
