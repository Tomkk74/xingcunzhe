...
function normalize(v: any) {
  ...
  p.season.started = p.season.started !== false; p.season.level = num(p.season.level ?? 1, 1, 20); p.season.xp = num(p.season.xp, 0, 999999999); p.season.totalXp = num(p.season.totalXp, 0, 999999999);
...
function runReward(p: any, a: any) { const endlessLayer = num(a.endlessLayer,0,1000), sig = JSON.stringify([num(a.time,0,86400),num(a.bossKills,0,100),num(a.level,1,300),!!a.win,endlessLayer]); if (p.lastRunSig === sig) return p; p.lastRunSig = sig; p.soulGold += rewardGold(p, a); p.soulCore += num(a.bossKills, 0, 80) + (a.win ? 2 : 0); if (a.win === true && endlessLayer === 0) p.rift.keys += 8; return p; }
function runXp(p: any, a: any) { const gain = Math.min(50000, Math.max(10, num(a.kills,0,20000) + num(a.bossKills,0,80) * 85 + Math.floor(num(a.time,0,86400) / 10) * 8 + (a.win ? 500 : 0) + num(a.endlessLayer,0,300) * 180)); addXp(p, gain); return p; }
...
function finishRift(p: any, a: any) { const ar = p.rift.activeRun, token = String(a.token || ''); if (!ar || ar.token !== token) throw new Error('秘境开局令牌无效或已结算'); const layer = num(a.layer, 1, RIFT_MAX), time = num(a.time, 0, 900), win = a.win === true; if (layer !== ar.layer) throw new Error('秘境层数不匹配'); const wall = Math.floor((Date.now() - Number(ar.startedAt || 0)) / 1000); if (win && (time < 20 || wall < 15)) throw new Error('通关时间异常'); const gainDust = win ? Math.round(8 + layer * 1.65) : Math.floor(layer / 4); const ticketDrop = win && layer >= 10 && stableChance(token, 30); p.rift.dust += gainDust; if (win) { p.rift.maxLayer = Math.max(p.rift.maxLayer, Math.min(RIFT_MAX, layer + 10)); if (ticketDrop) p.rift.keys++; const b = p.rift.best[layer]; if (!b || time < b.time) p.rift.best[layer] = { time, classId: String(a.classId || '').slice(0, 24), at: Date.now() }; } p.rift.lastResult = { token, layer, time, win, level: num(a.level,1,300), kills: num(a.kills,0,20000), bossKills: win ? 1 : 0, classId: String(a.classId || '').slice(0,24), at: new Date().toISOString() }; delete p.rift.activeRun; return p; }
